...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Step 2: Record response (INSERT; skip if already answered via UNIQUE constraint) ──
    ...
    // If the row already exists (unique constraint), skip session count updates
    // This prevents questions_answered from inflating past actual response count
    const wasInserted = !insertRes.error && insertRes.data && insertRes.data.length > 0;

    // ── Step 5: Pre-fetch next question from remaining IDs ──
    ...
    // Step 3: Update session counts (only if response was actually recorded)
    if (wasInserted) {
    ...
    // Step 4: Update daily count (free users only) — even on retry/duplicate
